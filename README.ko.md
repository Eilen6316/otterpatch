...
> 🦦 **O**ffice **T**ransforms · **T**racked · **E**dited & **R**eviewed · surgical **Patch** — 에이전트 기반의 검토 가능한 문서용 **세이프 커밋 레이어**.
...
에이전트가 파일을 직접 수정해서는 안 됩니다. OtterPatch에서 에이전트는 구조화된
...
npm test -w @otterpatch/core             # 어댑터 레지스트리
npm test -w @otterpatch/agent            # 의도 → ChangeSet(목 모델 + 8개 공급자 팩토리)
npm test -w @otterpatch/adapter-univer   # 의도 → ChangeSet → 외과적 .xlsx 되쓰기
npm test -w @otterpatch/adapter-drawio   # mxCell 연산 + 다이어그램 간 외과적 되쓰기
