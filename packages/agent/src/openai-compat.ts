 ...
 * 用 function-call 产出受约束的 propose_changeset。BYOK:apiKey 各厂商自带;baseURL 区分厂商。
 *
...
export class OpenAICompatModelClient implements ModelClient {
  ...
  async proposeChangeSet(req: ProposeRequest): Promise<ChangeSet> {
    ...
    if (!this.forcedTool) {
      messages.push({ role: 'user', content: '请只调用 propose_changeset 工具来完成上面的修改,不要用普通文字回答。' });
    }
    ...
    const res = await this.client.chat.completions.create({
      ...
      tool_choice: this.forcedTool ? { type: 'function', function: { name: PROPOSE_TOOL_NAME } } : 'auto',
    });
