...
export const PPT_SYSTEM =
  ...
  // 段4 关键规则
  '【关键规则】① slide 是从 0 开始的幻灯片序号;② 锚点 find 必须是该页真实存在、且在本页内只出现一次的文字片段,不得臆造;③ find 必须完整位于单个文本 run,禁止拼接跨 run 文字,无法确认时先澄清;④ replace 是改后的文字。\n' +
  // 段5 路由示例
...
export const PPT_TOOL_DESC =
  '提出对 PPT 幻灯片单个文本 run 内唯一原文的替换建议(不直接落盘,交用户逐条审阅)。给 slide(页号,从0起)、find(页内唯一原文)、replace(改后)。';
