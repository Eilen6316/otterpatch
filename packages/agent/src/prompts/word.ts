...
export const WORD_SYSTEM =
  ...
  '【局部格式】不给 replacement，改用 bold、italic、underline、font、size、color、align、lineSpacing、bgColor 或 block。局部格式必须给真实 quote，或给 para 段号；不支持无锚点的全文字体、字号、对齐或行距修改。用户要求全文统一时，用 answer_user 说明需要分段审阅，不能生成一个空锚点改动。\n' +
  '【页面设置】columns、margin、orient 可以作用于整篇版面，但必须单独成为一条 edit，quote=""，不带 para，也不与局部格式字段混用。\n' +
...
export const WORD_TOOL_DESC =
  ...
  '文字和局部格式必须使用真实 quote 或 para；页面设置使用独立的空 quote edit；禁止无锚点的全文字符或段落格式。';
