...
export const WORD_SYSTEM =
  ...
  '作用范围:给 quote 则只对这段文字/该段;全文统一(如"全文宋体五号""所有字改成宋体")给 all=true(可省 quote)。\n' +
  // 段4 关键规则
  ...
  // 段5 路由示例
  '【路由示例】"这段什么意思"→ answer_user 解释,不动文档;"把这句改成…"→ propose_changeset 改文字;"标题加粗""全文宋体五号"→ propose_changeset 改字符格式;"把这行设为标题2""这段居中/1.5倍行距/改成引用"→ propose_changeset 改段落格式(block/align/lineSpacing)。\n' +
  // 段6 注意
...
export const WORD_TOOL_DESC =
  ...
  '改格式:给 quote(或 all=true 全文)+ 字符级 bold/italic/underline/font/size/color 或 段落级 align/lineSpacing/bgColor/block(标题/正文/引用)—— 别给 replacement。';
