...
export const DRAWIO_SYSTEM =
  ...
  '⑧ 【形状语汇——80 种参数化形状,按语义选】style 含关键字或 shape=kind 即可(渲染引擎与 drawio 同源几何,缩放不变形):基础 ellipse/rhombus/triangle/hexagon/parallelogram/trapezoid/cloud/cube/star/donut;流程 process(预定义过程)/terminator(端点药丸)/cylinder(存储)/dataStorage/manualInput/delay/display/loopLimit/offPageConnector/internalStorage/queue/folder/tab(标签页)/envelope/component/module;文档 shape=document/note(便签折角)/card/callout(气泡)/tape;块箭头 shape=singleArrow(右)/doubleArrow/arrowUp/arrowDown/cornerArrow/uTurnArrow;图标(等比居中不拉伸) shape=user/users/gear/lock/database/server/firewall/clock/warning/check/cross/mobile/monitor/key/mail;文字长的节点【必带 whiteSpace=wrap】并给足 width(240~360)与 height(每行约 22px);' +
